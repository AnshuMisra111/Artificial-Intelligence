...
file = (open("/Users/anshuman786/Downloads/Sample_test_cases 2/input1.txt", "r")).readlines()
...
def run(rownumber, colnumber):
    ...
    #if rownumber==61 and colnumber==71:
        #print(temp1, dir1, temp2, dir2, temp4, dir3, temp3, dir4)
...
def diff(a,b):
    if (a=="Exit") or (a=="None") or (b=="Exit") or (b=="None"):
        return 0
    dict={"Walk Up":8,"Walk Down":7,"Walk Left":5,"Walk Right":4,"Run Up":3,"Run Down":2,"Run Left":1,"Run Right":0}
  ...
  while (len(traverse) != 0):
    ...
    if ((x >= 0) and (x < row)) and ((y >= 0) and (y < column)):
        ...
        if wall[x][y] == True:
            policy[x][y] = "None"
            list1 = []
            if (x + 1 >= 0) and (x + 1 < row) and (y >= 0) and (y < column) and (flag[x+1][y]==False): #and (counter[x+1][y]<20): #and (wall[x + 1][y] == False) and (
            ...
            list1 = []
            if (x >= 0) and (x < row) and (y - 1 >= 0) and (y - 1 < column)and (flag[x][y-1]==False): #and (counter[x][y-1]<20): #and (wall[x][y - 1] == False) and (
          ...
          if tstates[x][y][0]==True:
            ...
            if (x >= 0) and (x < row) and (y + 1 >= 0) and (y + 1 < column)and (flag[x][y+1]==False): #and (counter[x][y+1]<20):  # and (wall[x][y + 1] == False) and (
                ...
                #print("what is here", x, y, y-1,x-1,flag[x][y-1],flag[x-1][y])
                ...
                if (x + 1 >= 0) and (x + 1 < row) and (y >= 0) and (y < column) and (
                        flag[x + 1][y] == False): #and (counter[x+1][y]<20):  # and (wall[x + 1][y] == False) and (
                    # tstates[x + 1][y][0] == False) and (flag[x+1][y]==False):
                    # print("Enters1")
                    flag[x + 1][y] = True
                    newvalue, direction = utility(x + 1, y)
                ...
                if (x >= 0) and (x < row) and (y - 1 >= 0) and (y - 1 < column) and (
                        flag[x][y - 1] == False): #and (counter[x][y-1]<20):  # and (wall[x][y - 1] == False) and (
                    # tstates[x][y - 1][0] == False) and (flag[x][y-1]==False):
                    # print("Enters3")
                    newvalue, direction = utility(x, y - 1)
                    flag[x][y - 1] = True
